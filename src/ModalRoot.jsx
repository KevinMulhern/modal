...
export const ModalStackProvider = ({ children }) => {
    ...
    const push = (component, response, config, onClose, afterLeave) => {
        ...
        // In a single-modal world, this is always 0
        newModal.index = 0

        // 🔴 KEY CHANGE: replace the whole stack with just this modal
        setStack((prevStack) => {
            // Optional: clean up listeners on the old modal, if you care
            const previous = prevStack[0]
            if (previous) {
                Object.keys(previous.listeners).forEach((event) => {
                    previous.off(event)
                })
            }
